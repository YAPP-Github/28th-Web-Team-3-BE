...
interface OnboardingApi {
    ...
    @Operation(
        ...
        description = """
            온보딩 입력 스텝(onboarding_age·onboarding_finance·onboarding_assets·onboarding_period)에서 입력한 값을
            부분 저장(upsert)한다. 한 번에 전부 보내지 않고, 각 스텝을 넘어갈 때마다 그 스텝의 필드만 담아 호출한다.

            - 1/4 나이: `birthDate`
            - 2/4 월급·저축: `monthlySalaryManwon`, `monthlySavingManwon`
            - 3/4 순자산: `netWorthManwon`
            - 4/4 목표기간: `goalPeriodMonths`

            스텝마다 서버에 저장하므로 앱을 껐다 켜도 진행 상태가 복원된다.
            `monthlySavingManwon`이 `monthlySalaryManwon`을 초과하면 `400 INVALID_ONBOARDING_INPUT`.
            목표 확정(COMPLETED) 이후 월저축액·목표기간을 바꾸면, 확정해 둔 목표와 어긋나므로 목표가 무효화되고
            진행 중(IN_PROGRESS) 상태로 되돌아간다.
        """,
    )
    @ApiResponses(
    ...
    @Operation(
        summary = "온보딩 프로필 조회",
        description = """
            온보딩 화면 재진입 시(onboarding_intro에서 미완료 상태로 재접속 등) 지금까지 저장된 입력값과
            진행 상태(`IN_PROGRESS`/`COMPLETED`)를 반환한다. 클라이언트는 이 값으로 마지막에 머문 스텝부터 이어서 진행한다.
    ...
    @Operation(
        ...
        description = """
            '나는 잘하고 있을까?' 재무 분석 리포트 화면(onboarding_report) 데이터를 계산해 반환한다.
            저장된 프로필의 월급·월저축액·순자산·목표기간이 모두 있어야 하며, 하나라도 없으면 `409 ONBOARDING_INCOMPLETE`.

            응답 구성:
            - **시뮬레이션**: 현행 유지(baseline) vs 저축률 상향 시 예상 금액(simulation)과 그 차액. 연복리를 적용한 추정치이다.
            - **또래 비교**: 순자산 중앙값 대비 비율, 소득·소비 상위 백분위. 입력 월급은 세후 실수령액이므로 세전으로 보정해 통계와 비교한다.
            - **히스토그램**: 또래 소득·소비 분포와 사용자 위치 마커.
            - **종합 진단**: 자산·소득·소비 3축 조합(8분기)에 따른 맞춤 진단·조언 문구.
    ...
    @Operation(
        summary = "목표 금액안 조회",
        description = """
            '2가지 목표금액을 준비했어요' 목표 선택 화면(onboarding_goalselect) 데이터를 반환한다.
            저장된 월저축액·목표기간을 바탕으로 1안(확실하게)·2안(여유롭게) 두 목표안을 계산한다.

            각 안은 다음으로 구성된다.
            - **목표 증가분 범위**: 기존 저축 대비 추가로 더 모을 금액 범위(min~max)
            - **기간별 체크포인트**: 목표기간을 4등분한 시점별 목표 금액(막대그래프용)
            - **최종 카드**: 목표기간 종료 시점의 목표 금액
    ...
    @Operation(
        summary = "목표 확정",
        description = """
            목표 선택 화면의 '이 목표로 시작' 동작. 사용자가 고른 안(`PLAN_1` 확실하게 / `PLAN_2` 여유롭게)으로
            목표를 확정 저장하고, 온보딩을 완료(`COMPLETED`) 처리한다. 확정된 목표 금액·기간을 반환한다.
            이미 확정된 목표가 있으면 새 선택으로 대체된다. 월저축액·목표기간이 없으면 `409 ONBOARDING_INCOMPLETE`.
        """,
    )
    @ApiResponses(
