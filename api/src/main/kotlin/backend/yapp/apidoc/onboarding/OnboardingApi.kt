...
interface OnboardingApi {
    ...
    @Operation(
        ...
        description = "온보딩 입력 스텝(나이 / 월급·저축 / 순자산 / 목표기간)에서 입력한 값을 부분 저장(upsert)한다. " +
            "한 번에 전부 보내지 않고, 각 스텝을 넘어갈 때마다 그 스텝의 필드만 담아 호출한다.\n" +
            "1/4 나이: birthDate\n" +
            ...
            "월저축액이 월급을 초과하면 400(INVALID_ONBOARDING_INPUT). " +
    ...
    @Operation(
        summary = "온보딩 프로필 조회",
        description = "온보딩 화면 재진입 시(미완료 상태로 재접속 등) 지금까지 저장된 입력값과 진행 상태(IN_PROGRESS / COMPLETED)를 반환한다. " +
            "클라이언트는 이 값으로 마지막에 머문 스텝부터 이어서 진행한다. " +
    ...
    @Operation(
        ...
        description = "'나는 잘하고 있을까?' 재무 분석 리포트 화면(onboarding_report) 데이터를 계산해 반환한다. " +
    ...
    @Operation(
        summary = "목표 금액안 조회",
        description = "'2가지 목표금액을 준비했어요' 목표 선택 화면(onboarding_goalselect) 데이터를 반환한다. " +
