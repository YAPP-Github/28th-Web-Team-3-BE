...
interface OnboardingApi {
    ...
    @Operation(
        ...
        description = "온보딩 입력 스텝(나이 / 월급·저축 / 순자산 / 목표기간)에서 입력한 값을 부분 저장(upsert)한다. " +
            "한 번에 전부 보내지 않고, 각 스텝을 넘어갈 때마다 그 스텝의 필드만 담아 호출한다.<br>" +
            "1/4 나이: birthDate<br>" +
            "2/4 월급·저축: monthlySalaryManwon, monthlySavingManwon<br>" +
            "3/4 순자산: netWorthManwon<br>" +
            "4/4 목표기간: goalPeriodMonths<br>" +
    ...
    @Operation(
        ...
        description = "'나는 잘하고 있을까?' 재무 분석 리포트 화면(onboarding_report) 데이터를 계산해 반환한다. " +
            "저장된 프로필의 월급·월저축액·순자산·목표기간이 모두 있어야 하며, 하나라도 없으면 409(ONBOARDING_INCOMPLETE).<br>" +
            "응답 구성<br>" +
            ...
            "종합 진단: 자산·소득·소비 3축 조합(8분기)에 따른 맞춤 진단·조언 문구.<br>" +
