...
| 필드 | 의미 |
|---|---|
| `answerType` | `SINGLE_CHOICE`, `MULTI_CHOICE`, `NUMBER`, `KEYED_NUMBER`, `FREQUENCY_RANGE`, `KEYED_FREQUENCY_RANGE` |
...
| `frequencyRangeOptions` | 단일 범위형 질문의 선택지 코드·표시 문구·경계 |
| `frequencyRangeRules` | 선행 선택 코드별 범위형 선택지와 단위 |
...
| `FREQUENCY_RANGE` | `String?` | 서버가 내려준 범위 선택지 코드 1개. 조건에 따라 `null` |
| `KEYED_FREQUENCY_RANGE` | `List<Object>` | 선행 질문에서 선택한 각 코드와 범위 선택지 코드를 한 쌍으로 전달 |
...
| 4 | `HOBBY_FREQUENCIES`<br>선택한 항목에 평소 얼마나 자주 결제하나요? | `hobby.frequencies: List<{spendingType, frequencyRange}>` | 항목별 범위 선택 | `SUBSCRIPTION`: `ONE` 1개<br>`TWO` 2개<br>`THREE_OR_MORE` 3개 이상<br>그 외: `ONE_TO_TWO` 1~2회<br>`THREE_TO_FOUR` 3~4회<br>`FIVE_TO_SIX` 5~6회<br>`SEVEN_OR_MORE` 7회 이상 | key 집합과 개수가 `spendingTypes`와 정확히 일치<br>`DO_NOT_REDUCE`이면 `[]` |
...
| 3 | `LIVING_FREQUENCIES`<br>해당 소비는 얼마나 자주 발생하나요? | `living.frequencies: List<{area, frequencyRange}>` | 항목별 범위 선택 | `SUBSCRIPTION`: `ONE` 1개<br>`TWO` 2개<br>`THREE_OR_MORE` 3개 이상<br>그 외: `ONE_TO_TWO` 1~2회<br>`THREE_TO_FOUR` 3~4회<br>`FIVE_TO_SIX` 5~6회<br>`SEVEN_OR_MORE` 7회 이상 | key 집합과 개수가 `areas`와 정확히 일치<br>`UNKNOWN`이면 `[]` |
...
| 범위형 빈도 | `frequencyRangeOptions` 또는 `frequencyRangeRules`에서 현재 선택 항목의 코드만 표시·제출 |
