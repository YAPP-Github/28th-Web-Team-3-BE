...
The production workflow keeps worker container concurrency at `1` and defines the default bounded-throughput stage through deployment variables: queue dispatch rate=`4`, queue concurrent dispatches=`4`, worker max instances=`4`, and worker Hikari maximum pool size=`2` (minimum idle=`0`). This accepts a burst of requests into Cloud Tasks while processing no more than four mission generations at once. The worker needs a pool size of two during Flyway startup; at four worker instances, the database connection upper bound is eight. It also uses a 60-second provider transport timeout without SDK retries, a 90-second Cloud Tasks dispatch deadline, and a 120-second Cloud Run worker timeout. The worker's service-level minimum of `1` and startup CPU boost remain required.

Do not raise the bounded-throughput defaults above `4` without separate deployment and load-test approval. Before increasing queue rate, queue concurrency, and worker max instances together, verify an approved Gemini quota/cost limit and the full DB/PgBouncer connection budget. The worker Hikari maximum pool must stay within that approved budget.
