...
WHERE id IN (
    ...
    WHERE question_code IN ('MEAL_FREQUENCY', 'TRANSPORT_FREQUENCY', 'HOBBY_FREQUENCIES', 'LIVING_FREQUENCIES')
...
SET range_code = CASE
        WHEN unit_code = 'SUBSCRIPTION_COUNT' AND numeric_value = 1 THEN 'ONE'
        WHEN unit_code = 'SUBSCRIPTION_COUNT' AND numeric_value = 2 THEN 'TWO'
        WHEN unit_code = 'SUBSCRIPTION_COUNT' THEN 'THREE_OR_MORE'
    ...
    END,
    numeric_value = CASE
        WHEN unit_code = 'SUBSCRIPTION_COUNT' AND numeric_value = 1 THEN 1
        WHEN unit_code = 'SUBSCRIPTION_COUNT' AND numeric_value = 2 THEN 2
        WHEN unit_code = 'SUBSCRIPTION_COUNT' THEN 3
